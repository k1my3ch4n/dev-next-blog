...
<a href='https://blog.k1my3ch4n.xyz/blog/ssr-ssg-ssr' target="_blank">
   <img src='https://img.shields.io/badge/post-SSR_→_SSG_→_SSR-06B6D4?style=for-the-badge&labelColor=4C566A'>
</a>
...
Monorepo 의 Blog 패키지 입니다. 학습하고 경험한 것들을 남기는 블로그 입니다. 메인 페이지와 블로그 목록 페이지, post 페이지로 이루어져 있습니다. 게시글 데이터는 정적 상수(constants)로 관리합니다.
...
<details open>
<summary><strong>SQL → Constants 전환</strong></summary>

### 변경 사항 개요

PostgreSQL 런타임 조회를 완전히 제거하고, 게시글 데이터를 `src/entities/post/constants.ts`의 정적 배열로 관리하도록 전환했습니다.
...
| DB 연결     | `pg` Connection Pool     | 불필요                 |
| 환경변수    | DB_USER / DB_HOST 등 5개 | 없음                   |

#### 제거된 파일

- `src/shared/lib/db.ts` — pg Pool, getPosts/getPost/getAllTags 삭제
- `pg`, `@types/pg`, `dotenv` 패키지 제거

#### 변경된 파일

- `src/entities/post/constants.ts` — `BLOG_POSTS` 배열 및 3개 함수 추가
- `src/shared/lib/index.ts` — db.ts re-export → `@entities/post` re-export로 교체

#### 유지된 파일

`getBlogData`, `getHomeData`, `getPostData` 및 모든 페이지/위젯은 함수 시그니처가 동일하므로 변경 없음.
...
없음. 게시글 데이터는 `src/entities/post/constants.ts`에서 정적으로 관리됩니다.
