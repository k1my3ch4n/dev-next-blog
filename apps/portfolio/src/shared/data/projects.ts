import { TwinnyLogo, MainLogo, HRAHLogo, AIGithubActions } from "@shared/assets/images";
...
export const PROJECT_DETAILS: Record<string, ProjectDetail> = {
  syngrid: {
    ...
    techStack: ["React", "WebSocket", "Proxy Server", "Claude Code", "GCP Cloud Run"],
  ...
  userRobot: {
    ...
    description: "사용자를 위한 실시간 로봇 관제 대시보드 및 엑셀 기반 WMS 작업 관리 시스템.",
    shortDescription: "실시간 대시보드 · MSW · 빌드 72%↓ · 테스트 87%",
    thumbnail: TwinnyLogo,
    type: "career",
    period: "2023.01 — 2024.04",
    techStack: ["React", "TypeScript", "Vite", "MSW", "Jest", "Storybook", "GitHub Actions"],
    gradient: "linear-gradient(135deg, #1a1a2e, #312e81)",
    stories: [
      {
        title: "실시간 대시보드 & WMS 개발",
        problem: "현장에서 로봇의 위치, 상태, 작업 현황을 실시간으로 확인하기 힘듦",
        solution: "실시간 대시보드를 개발해 로봇 및 작업 상태를 실시간으로 확인 가능",
        extension: "엑셀 기반 WMS 작업 생성 및 지시 기능으로 현장에서 쉽게 작업 시작/확인 가능",
      },
      {
        title: "MSW 기반 프론트엔드 주도 개발 체계 구축",
        problem: "서버 API 개발 지연 시 프론트 작업이 블로킹, 디자인팀은 실제 데이터 없이 UI 검증 불가",
        solution: "MSW 도입으로 API 명세서 기반 프론트 선행 개발 → 프론트 주도 프로세스 정립",
        extension: "MSW 활용 Storybook 도입 → 디자인팀이 실제 데이터 흐름 반영된 UI 직접 확인 가능",
      },
      {
        title: "빌드 시간 72% 단축 (CRA → Vite + Monorepo)",
        problem: "CRA 기반 빌드가 2분 이상 소요, 비슷한 레포 간 공통 코드 중복",
        solution: "CRA → Vite 마이그레이션으로 빌드 시간 72% 단축 (2:20 → 0:40)",
        extension: "Yarn Workspace Monorepo 적용, 공통 hook/component 모듈화, CI/CD 자동화",
      },
      {
        title: "테스트 커버리지 20% → 87% 달성",
        problem: "낮은 테스트 커버리지로 리팩토링 시 사이드 이펙트 검증 불가",
        solution: "Jest + React Testing Library + MSW 조합한 테스트 전략 수립 및 실행",
    ...
    stories: [
      {
        ...
        solution: "JavaScript → TypeScript 전환 (95% 이상), 30개 이상 유효성 검사 모듈화",
  ...
  hackerRankAiHelper: {
    ...
    techStack: ["Next.js", "Gemini API", "Zustand", "Tailwind CSS", "GCP Cloud Run"],
    ...
    stories: [
      {
        ...
        solution: "Gemini API 사용해 문제 자동 번역, 요약, 단계별 힌트 제공 기능 구현",
        extension: "Zustand 전역 상태 관리 및 모바일 반응형 서비스 추가, GCP Cloud Run 배포",
      },
    ],
    relatedLinks: [
      { label: "Notion", url: LINKS.HRAH_NOTION },
      { label: "GitHub", url: "https://github.com/k1my3ch4n/HackerRankAIHelper" },
    ],
  },
  monorepo: {
    id: "monorepo",
    ...
    techStack: ["Next.js", "GraphQL", "Apollo Server", "PostgreSQL", "Docker", "GitHub Actions"],
    gradient: "linear-gradient(135deg, #2e1a24, #5c2d44)",
    stories: [
      {
        title: "CSR 한계 → ISR 기반 렌더링 전략 재설계",
        problem: "CSR 기반이라 검색 엔진에 콘텐츠가 노출되지 않음",
        solution: "Vite → Next.js 마이그레이션, generateStaticParams + revalidate ISR 적용",
        extension: "Sass → Tailwind CSS 적용",
      },
      {
        title: "프론트엔드만으로는 한계 → 백엔드 직접 구축",
        problem: "블로그 포스트 관리를 위한 서버가 필요함",
        solution: "PostgreSQL + Apollo Server + GraphQL로 데이터 레이어 직접 설계 및 구축",
  ...
  aiGithubActions: {
    ...
    description: "Claude/Gemini 기반 AI 코드 리뷰 Action & 테스트 헬퍼 Action. GitHub Marketplace 공개.",
    ...
    stories: [
      {
        ...
        solution: "Claude/Gemini 사용해 코드 리뷰 Action과 테스트 헬퍼 Action 개발",
        extension: "GitHub Marketplace에 공개 후 개인 프로젝트에 적용",
      },
    ],
    relatedLinks: [
      { label: "AI-Code-Reviewer", url: "https://github.com/marketplace/actions/ai-code-reviewer-by-k1my3ch4n" },
      { label: "AI-Test-Helper", url: "https://github.com/marketplace/actions/ai-test-helper" },
    ],
